...
public class DetailTransaksiDialog extends AppCompatDialog {
    public interface OnDetailTransaksiListener {
        void onResult(int jum);
    }

    private final Keramik keramik;
    private final OnDetailTransaksiListener callback;

    public DetailTransaksiDialog(Context context, Keramik keramik, OnDetailTransaksiListener callback) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.dialog_detail_transaksi);
        ...
        findViewById(R.id.tambah_barang).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                EditText ed = findViewById(R.id.jumlah);
                if (ed.getText().toString().isEmpty()) {
                    Toast.makeText(getContext(), "Jumlah masih kosong !", Toast.LENGTH_LONG).show();
                    return;
                } else if (Integer.parseInt(ed.getText().toString()) == 0) {
                    Toast.makeText(getContext(), "Jumlah Tidak boleh 0 !", Toast.LENGTH_LONG).show();
                    return;
                }
                if (Integer.parseInt(ed.getText().toString()) > keramik.getStock()) {
                    Toast.makeText(getContext(), "Jumlah lebih besar dari Stock !", Toast.LENGTH_LONG).show();
                    return;
                }
                callback.onResult(Integer.parseInt(ed.getText().toString()));
                dismiss();
            }
        });
    }
